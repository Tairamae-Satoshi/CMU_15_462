...
namespace CMU462 {
	...
	Ray Camera::generate_ray(double x, double y) const {
		...
		double imageAspectRatio = (double)screenW / (double)screenH;
		double scale = tan(radians(vFov/2));
		double px = (2 * x - 1) * imageAspectRatio * scale;
		double py = (2 * y - 1) * scale;
 
		return Ray(pos, c2w * Vector3D(px, py, -1));
