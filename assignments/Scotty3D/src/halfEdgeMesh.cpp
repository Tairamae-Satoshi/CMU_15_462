...
namespace CMU462 {
	...
	float Vertex::laplacian() const {
		...
		vector<VertexIter> vs_adj;
		HalfedgeIter h = _halfedge;
		do
		{
			h = h->twin();
			vs_adj.push_back(h->vertex());
			h = h->next();
		} while (h != halfedge());
		...
		{
			Vector3D vi_pos = position;
			Vector3D vj_pos = vs_adj[j]->position;
			Vector3D alpha_pos = vs_adj[(j - 1 + n) % n]->position;
			Vector3D beta_pos = vs_adj[(j + 1) % n]->position;

			double cot_alpha = dot(vi_pos - alpha_pos, vj_pos - alpha_pos) / cross(vi_pos - alpha_pos, vj_pos - alpha_pos).norm();
			double cot_beta = dot(vi_pos - beta_pos, vj_pos - beta_pos) / cross(vi_pos - beta_pos, vj_pos - beta_pos).norm();
			delta_u += (cot_alpha + cot_beta) * (vs_adj[j]->offset - offset);
		}
