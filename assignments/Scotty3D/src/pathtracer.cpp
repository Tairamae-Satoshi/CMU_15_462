...
//#define ENABLE_RAY_LOGGING 1
	...
	Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
		...
		for (size_t i = 0; i < ns_aa; i++)
		{
			...
			Vector2D p = Vector2D((x + 0.5) / (double)camera->width(), (y + 0.5) / (double)camera->height());
			spectrum += trace_ray(camera->generate_ray(p.x, p.y));
		}

		return Spectrum(spectrum.r / ns_aa, spectrum.g / ns_aa, spectrum.b / ns_aa);
	}
