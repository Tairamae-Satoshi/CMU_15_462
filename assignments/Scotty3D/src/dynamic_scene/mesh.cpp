...
namespace CMU462 {
	namespace DynamicScene {
		...
		void Mesh::linearBlendSkinning(bool useCapsuleRadius) {
			...
			for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++)
			{
				...
				for (auto joint: skeleton->joints)
				{
					...
					// frame when no transformations have been applied to the skeleton (bind pose, vertex bind position)
					Matrix4x4 T = joint->getBindTransformation();
					Vector3D bindPosition = T.inv() * v->bindPosition;

					// Find where this vertex would end up (in world coordinates) if it were transformed along with bone j
					T = joint->getTransformation();
					Vector3D endPosition = T * bindPosition;

					// Find the closest point on joint j's bone segment(axis) and compute the distance to this closest point
					Vector3D axis = joint->axis;
					const float l2 = axis.norm2();
					double distance = 0.0;
					const float t = max(0.0, min(1.0, dot(bindPosition, axis) / l2));
					const Vector3D projection = axis * t;
					distance = l2 == 0.0 ? bindPosition.norm() : (bindPosition - projection).norm();
					

					// Store the transformed location and distance for each j in an LBSInfo structure, which has been defined				
					if (useCapsuleRadius && distance > joint->capsuleRadius)
						continue;
					
					LBSInfo info;
					info.blendPos = endPosition;
					info.distance = distance;
					LBSInfos.push_back(info);
					sum_dist_inv += 1 / distance;
					
				}
				
				// Compute the resulting position of the vertex by doing a weighted average of the positions from each bone,
				// as stored in the LBSInfo structures. The weights for the weighted average should be the inverse distance 
				// to the joint, so closer bones have a stronger influence.

				if (LBSInfos.size() > 0) {
					v->position = Vector3D(0, 0, 0);
					for (auto LBSInfo : LBSInfos) {
						double wij = 1.0 / LBSInfo.distance / sum_dist_inv;
						v->position += wij * LBSInfo.blendPos;
					}
				}
		...
		void Mesh::forward_euler(float timestep, float damping_factor) {
			...
			vector<float> delta_us;
			for (auto v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
				delta_us.push_back(v->laplacian());
			}

			size_t i = 0;
			for (auto v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
				v->offset += timestep * v->velocity;
				v->velocity += timestep * delta_us[i++] - damping_factor * timestep * v->velocity;
