...
namespace CMU462 {
	namespace DynamicScene {
		...
		void Joint::calculateAngleGradient(Joint* goalJoint, Vector3D q) {
			...
			vector<Joint*> joints;
			for (auto j = goalJoint;j != this->parent; j = j->parent)
			{
				joints.push_back(j);
			}

			if (joints.size()==0||joints.back()!=this)
			{
				return;
			}

			Vector3D endPos = goalJoint->getEndPosInWorld();
			Vector3D delta = endPos - q;
			vector<Vector3D> axes;
			axes.resize(3);

			for (auto iter = joints.rbegin(); iter!=joints.rend(); iter++)
			{
				Vector3D basePos = (*iter)->getBasePosInWorld();
				(*iter)->getAxes(axes);
				Vector3D p = endPos - basePos;

				(*iter)->ikAngleGradient.x += dot(cross(axes[0], p), delta);
				(*iter)->ikAngleGradient.y += dot(cross(axes[1], p), delta);
				(*iter)->ikAngleGradient.z += dot(cross(axes[2], p), delta);
			}
		}
		...
		Vector3D Joint::getEndPosInWorld() {
			...
			*/
			Matrix4x4 T = getTransformation();
			T = T * getRotation() * Matrix4x4::translation(axis);
