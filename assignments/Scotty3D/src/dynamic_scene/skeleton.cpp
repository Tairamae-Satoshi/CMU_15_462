...
namespace CMU462 {
	namespace DynamicScene {
		...
		void Skeleton::reachForTarget(map<Joint*, Vector3D> targets, double time) {
			...
			{
				for (auto j = target.first; j != root; j=j->parent)
				{
					j->ikAngleGradient = 0;
					joints.insert(j);
				}
			...
			while (n-- > 0)
			{
				for (auto target:targets)
				{
					root->calculateAngleGradient(target.first, target.second);
				}
				for (auto j : joints)
				{
					Vector3D newR = j->rotation - 0.1 * j->ikAngleGradient;
					j->rotation = newR;
		...
		Joint* Skeleton::createNewJoint(Joint* parent, Vector3D endPos) {
			...
			Matrix4x4 T = parent == nullptr ? Matrix4x4::identity() : newJoint->getTransformation();
