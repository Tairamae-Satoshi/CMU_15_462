...
class Halfedge : public HalfedgeElement {
 public:
  ...
  HalfedgeIter pre()
  {
	  ...
	  return h;
  }  ///< access the previous half edge (comst iterator)
...
class Face : public HalfedgeElement {
 public:
  ...
  /**
   * Returns all halfedge of this face
   */
...
class Vertex : public HalfedgeElement {
 public:
  ...
  /**
   * returns adjacent vertices of this vertex
   */
  ...
  /**
   * returns adjacent edges of this vertex
   */
  ...
  /**
   * returns adjacent faces of this vertex
   */
  ...
  vector<HalfedgeIter> outHalfedges() {
	  vector<HalfedgeIter> hs;
	  HalfedgeIter h = halfedge();
	  do
	  {
  ...
  vector<HalfedgeIter> inHalfedges() {
	  vector<HalfedgeIter> hs;
	  HalfedgeIter h = halfedge();
	  do
	  {
  ...
  HalfedgeIter findHalfedgeOn(FaceIter f0)
  {
	  HalfedgeIter h = halfedge();
	  do
	  {
...
class Edge : public HalfedgeElement {
  ...
  */
  vector<EdgeIter> getAdjEdges() {
	  VertexIter v0 = halfedge()->vertex();
	  VertexIter v1 = halfedge()->twin()->vertex();
	  vector<EdgeIter> adjes = v0->adjEdges();
	  vector<EdgeIter> adjes_temp = v1->adjEdges();
	  adjes.insert(adjes.end(), adjes_temp.begin(), adjes_temp.end());
	  return adjes;
  }
  ...
  * return its length
  */
  double length(){ return (halfedge()->vertex()->position - halfedge()->twin()->vertex()->position).norm(); }
  ...
  /**
   * Check if one side of this edge contains only a vertex
   */
  bool isSingle() const
  {
	  HalfedgeCIter h0 = halfedge();
	  ...
	  return (h0->next() == h1 || h1->next() == h0) ? true : false;
  };
...
class HalfedgeMesh {
 public:
   ...
   */
   ...
   /**
	  * Check whether the returning edgeiter is valid
	  */
  bool isValid(EdgeIter e);

   /**
	 * Find a halfedge that points to v0 in the face f0 , returning a pointer to the found halfedge
	 */
  HalfedgeIter findHalfedgeTo(VertexIter v0, FaceIter f0);

  /**
	* Find a halfedge that start from v0 in the face f0, returning a pointer to the found halfedge
	*/
  HalfedgeIter findHalfedgeFrom(VertexIter v0, FaceIter f0);

   /**
	* Connect two vertices, returning a pointer to the new edge
	*/
  EdgeIter connectVertex(HalfedgeIter h_v0, HalfedgeIter h_v1);

  /**
	* Insert a new vertice in e0, returning the new vertice
	*/
  VertexIter insertVertice(double ratio, HalfedgeIter h);

  /**
	* Insert a new vertice connecting v0 on f0, returning the new vertice
	*/
  VertexIter insertVertice(HalfedgeIter hv0);

  /**
	* Find a edge that connects v0 and v1, returning a pointer to the edge
	*/
  EdgeIter findEdge(VertexIter v0, VertexIter v1);
